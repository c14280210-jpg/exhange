...
async function fetchRates() {
    ...
    try {
        ...
        for (const [key, config] of Object.entries(CONFIG.CURRENCIES)) {
            ...
            if (found && found.rateBuy) {
                rates[key] = {
                    buy: found.rateBuy,
                    sell: found.rateSell || found.rateBuy,
                    cross: found.rateCross || found.rateBuy,
                    timestamp: Date.now()
                };
            } else {
                hasError = true;
        ...
        if (hasError && Object.values(rates).every(r => r === null)) {
...
function updateDisplay(rates) {
    for (const [currency, data] of Object.entries(rates)) {
        ...
        if (!rateElement || !changeElement) continue;
        ...
        const currentRate = data.buy;
        const prevRate = state.previousRates[currency] || currentRate;
        
        // Анімація оновлення
        rateElement.classList.remove('updating');
        void rateElement.offsetWidth; // тригер перемальовки
        ...
        // Визначаємо тип зміни та анімуємо бегемотика
        ...
        state.previousRates[currency] = currentRate;
    }
...
function updateCountdownDisplay() {
    ...
    if (el) {
        el.textContent = `Наступне оновлення: ${Math.ceil(state.countdown)}с`;
    }
}
...
function createHippo() {
    ...
    const watchBody = new THREE.Mesh(
        new THREE.CylinderGeometry(0.1, 0.1, 0.035, 16), 
        goldMat
    );
...
// ===== 7. АНІМАЦІЇ БЕГЕМОТИКА =====
function animateHippo(type) {
    if (!hippo) return;
    currentAnimation = type;
    
    // Скидаємо позицію для нової анімації
    switch(type) {
        case 'happy':
        ...
        default:
            // Спокійне погойдування
            // (реалізовано в renderLoop)
            break;
    }
}

// Спрощена функція анімації (без GSAP)
